...
/* message */
$(".link_message_show").on("click", function(e){
    e.preventDefault();
    var row_current = $(this).parents("tr");
    var row_next = row_current.next("tr");
    if(row_current.attr("data-status")==1){
        row_next.attr("data-open",parseInt(row_next.attr("data-open"))^1);
    }else{
        $.ajax({
            url:url+"admin/message/status/edit?id="+$(this).attr("data-id"),
            type:"post",
            dataType:"json",
            success:function(response){
                if(response.status==1){
                    row_current.attr("data-status",1);
                    row_next.attr("data-open",parseInt(row_next.attr("data-open"))^1);
                }
            }
        });
...
$("#link_dog_create").on("click",function(e){
    ...
    $("#form_new-existing_dog").attr("action",url+"admin/dog/create");
...
$(".link_dog_show_edit").on("click",function(e){
    ...
    $.ajax({
        ...
        success:function(response){
            ...
            $("#form_new-existing_dog").attr("action",url+"admin/dog/edit");
            $("#formWrapper input[name=id]").remove();
            ...
            for(var i=0;i<response["resources"].length;i++){
                ...
                path = url+"assets/"+resource['type']+"/dog/"+resource['body'];
                $("#group_files").prepend(
                    "<div class='sf-file-container boxed'>"+
...
$(".link_dog_change_status").on("click",function(e){
    ...
    var id = $(this).attr("data-id");
    $.ajax({
        url:url+"admin/dog/_status-edit",
        type:'post',
        dataType:'json',
        data:{'id':id},
        success:function(response){
